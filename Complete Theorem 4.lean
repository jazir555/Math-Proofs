...
import Mathlib.Analysis.NormedSpace.Matrix -- Crucial for NormedAlgebra (Matrix n n Î±)
...
import Mathlib.LinearAlgebra.Matrix.Eigenvalues -- For IsEigenValue, HasEigenVector
import Mathlib.Data.Matrix.Invertible -- For Pâ»Â¹ and IsUnit det
import Mathlib.FieldTheory.IsAlgClosed.Eigenvalues -- For Eigenvalues exist over C
import Mathlib.LinearAlgebra.Matrix.IsDiag -- For diagonalizability predicate
import Mathlib.LinearAlgebra.Basis -- For Basis.mk
import Mathlib.LinearAlgebra.FiniteDimensional -- For finrank
import Mathlib.LinearAlgebra.Dimension.Fin -- For finrank_fin_fun
import Mathlib.LinearAlgebra.Span -- For span_eq_top
import Mathlib.RingTheory.Polynomial.Basic
import Mathlib.RingTheory.Polynomial.Roots
import Mathlib.Data.Polynomial.Splits -- For Splits
import Mathlib.RingTheory.Polynomial.Squarefree -- For Squarefree
...
open Matrix Finset Real Complex Pi Topology Module Submodule Basis Polynomial -- Added Filter, Polynomial

/- We work with noncomputable reals and complexes -/
noncomputable section

/-! # Part 1: Model Definitions and Basic Properties (Uniform and Inhomogeneous) -/
section ModelAndDefs

variable {N : â„•} (hN : 0 < N)

-- Define parameter space P = (beta, K_vec, mu_vec)
abbrev KSpace (N : â„•) := EuclideanSpace â„ (Fin N)
abbrev MuSpace (N : â„•) := EuclideanSpace â„ (Fin N)
abbrev FullParamSpace (N : â„•) := â„ Ã— KSpace N Ã— MuSpace N

-- Instances for Normed Space and other types
instance (N : â„•) : NormedAddCommGroup (FullParamSpace N) := inferInstance
instance (N : â„•) : NormedSpace â„ (FullParamSpace N) := inferInstance
...
instance : NormedAlgebra â„ (Matrix (Fin 2) (Fin 2) â„‚) := Matrix.normedAlgebra
instance : CompleteSpace â„‚ := Complex.completeSpace
instance : CompleteSpace â„ := Real.completeSpace

-- Helper Functions
@[simp] def boolToReal (b : Bool) : â„ := if b then 1.0 else 0.0
@[simp] def fin2ToBool (i : Fin 2) : Bool := i.val == 1
@[simp] def fin2ToReal (i : Fin 2) : â„ := boolToReal (fin2ToBool i)
abbrev Config (N : â„•) := Fin N â†’ Bool
abbrev Path (N : â„•) := Fin N â†’ Fin 2
instance : Fintype (Config N) := Pi.fintype
instance : Fintype (Path N) := Pi.fintype
instance {n : â„•} : DecidableEq (Fin n â†’ Fin 2) := Pi.decidableEqPiFintype

/- Hamiltonian Definition (dependent on K, mu vectors) -/
@[simp]
def latticeGasH_local_K (k : Fin N) (config : Config N) (K_vec : KSpace N) (mu_vec : MuSpace N) : â„ :=
  let n_k := config k; let n_kp1 := config (Fin.cycle hN k)
  - (K_vec k) * (boolToReal n_k) * (boolToReal n_kp1) - (mu_vec k) * (boolToReal n_k)

@[simp]
def latticeGasH_K (config : Config N) (K_vec : KSpace N) (mu_vec : MuSpace N) : â„ :=
  Finset.sum Finset.univ (fun (k : Fin N) => latticeGasH_local_K N hN config k K_vec mu_vec)

/- Z_ED Definition (dependent on beta, K, mu) -/
@[simp]
def Z_ED_K (beta : â„) (K_vec : KSpace N) (mu_vec : MuSpace N) : â„ :=
...
@[simp]
def T_local_exponent_full (p : FullParamSpace N) (i : Fin N) (idx1 idx2 : Fin 2) : â„ :=
...
@[simp]
def T_local_full (p : FullParamSpace N) (i : Fin N) : Matrix (Fin 2) (Fin 2) â„‚ :=
  Matrix.ofFn fun idx1 idx2 => Complex.exp (â†‘(T_local_exponent_full N hN p i idx1 idx2) : â„‚)

@[simp]
def T_prod_full (p : FullParamSpace N) : Matrix (Fin 2) (Fin 2) â„‚ :=
  let matrices := List.ofFn (fun i : Fin N => T_local_full N hN p i)
  List.prod matrices.reverse -- M_{N-1} * ... * M_0

@[simp]
def Z_TM_full (p : FullParamSpace N) : â„‚ := Matrix.trace (T_prod_full N hN p)

-- Log Z defined carefully using Z_ED for positivity proof later
@[simp]
def log_Z_tm_full (p : FullParamSpace N) : â„ :=
  let Z_re := (Z_TM_full N hN p).re
  if h : 0 < Z_ED_K N hN p.1 p.2.1 p.2.2 then Real.log Z_re else 0 -- Use Z_ED > 0

-- Free Energy
@[simp]
def F_TM (p : FullParamSpace N) : â„ := -(1 / p.1) * log_Z_tm_full N hN p

-- Domain beta > 0
def U_domain (N : â„•) : Set (FullParamSpace N) := { p | 0 < p.1 }
...
def configPathEquiv : Equiv (Config N) (Path N) where toFun := configToPath N; invFun := pathToConfig N; left_inv := by intro c; funext i; simp; split_ifs <;> rfl; right_inv := by intro p; funext i; simp; cases hi : p i; simp [Fin.val_fin_two, hi]; rfl

-- Path exponent sum definition
def path_exponent_sum_full (p : FullParamSpace N) (path : Path N) : â„ := Finset.sum Finset.univ fun (i : Fin N) => T_local_exponent_full N hN p i (path i) (path (Fin.cycle hN i))

-- Define Expectation Values (using Z_ED for clarity)
def expectation_ni (beta : â„) (K_vec : KSpace N) (mu_vec : MuSpace N) (i : Fin N) : â„ := (1 / Z_ED_K N hN beta K_vec mu_vec) * Finset.sum Finset.univ fun (c : Config N) => (boolToReal (c i)) * Real.exp (-beta * latticeGasH_K N hN c K_vec mu_vec)
def expectation_nn (beta : â„) (K_vec : KSpace N) (mu_vec : MuSpace N) (i : Fin N) : â„ := (1 / Z_ED_K N hN beta K_vec mu_vec) * Finset.sum Finset.univ fun (c : Config N) => (boolToReal (c i) * boolToReal (c (Fin.cycle hN i))) * Real.exp (-beta * latticeGasH_K N hN c K_vec mu_vec)

-- Define log Z as function of K and mu separately (fixing other params)
def log_Z_of_K (beta : â„) (mu_vec : MuSpace N) (K_vec : KSpace N) : â„ := log_Z_tm_full N hN (beta, K_vec, mu_vec)
def log_Z_of_mu (beta : â„) (J_vec : KSpace N) (mu_vec : MuSpace N) : â„ := log_Z_tm_full N hN (beta, J_vec, mu_vec)
...
variable {N : â„•} (hN : 0 < N) (p : FullParamSpace N) (config : Config N) (path : Path N)
...
lemma matrix_mul_real_entries {n m pâ‚ : Type*} [Fintype n] [Fintype m] [DecidableEq m] {A : Matrix n m â„‚} {B : Matrix m pâ‚ â„‚} (hA : âˆ€ i j, (A i j).im = 0) (hB : âˆ€ i j, (B i j).im = 0) : âˆ€ i j, ((A * B) i j).im = 0 := by intros i j; simp only [Matrix.mul_apply, Complex.sum_im]; apply Finset.sum_eq_zero; intro k _; let z1 := A i k; let z2 := B k j; have hz1 : z1.im = 0 := hA i k; have hz2 : z2.im = 0 := hB k j; simp [Complex.mul_im, hz1, hz2]
lemma matrix_list_prod_real_entries {n : Type*} [Fintype n] [DecidableEq n] (L : List (Matrix n n â„‚)) (hL : âˆ€ M âˆˆ L, âˆ€ i j, (M i j).im = 0) : âˆ€ i j, ((List.prod L) i j).im = 0 := by induction L with | nil => simp [Matrix.one_apply]; intros i j; split_ifs <;> simp | cons M t ih => simp only [List.prod_cons]; have hM : âˆ€ i j, (M i j).im = 0 := hL M (List.mem_cons_self M t); have ht : âˆ€ M' âˆˆ t, âˆ€ i j, (M' i j).im = 0 := fun M' h' => hL M' (List.mem_cons_of_mem M h'); apply matrix_mul_real_entries hM (ih ht)
lemma T_total_is_real_entries : âˆ€ i j, (T_prod_full N hN p i j).im = 0 := by unfold T_prod_full; apply matrix_list_prod_real_entries; intro M hM_rev i j; simp only [List.mem_reverse] at hM_rev; simp only [List.mem_map, List.mem_ofFn] at hM_rev; obtain âŸ¨k', _, hk_eqâŸ© := hM_rev; rw [â† hk_eq]; apply T_local_is_real N hN p k' i j
...
lemma sum_exponent_eq_neg_H_full : path_exponent_sum_full N hN p (configToPath N config) = -p.1 * (latticeGasH_K N hN config p.2.1 p.2.2) := by let beta := p.1; let K_vec := p.2.1; let mu_vec := p.2.2; dsimp [path_exponent_sum_full, latticeGasH_K, T_local_exponent_full, latticeGasH_local_K]; simp_rw [Finset.sum_mul, Finset.mul_sum, Finset.sum_neg_distrib, mul_add]; rw [Finset.sum_add_distrib, Finset.sum_add_distrib]; let term3 := fun i : Fin N => beta * (mu_vec (Fin.cycle hN i) / 2) * fin2ToReal (if config (Fin.cycle hN i) then 1 else 0); let term2 := fun i : Fin N => beta * (mu_vec i / 2) * fin2ToReal (if config i then 1 else 0); let e : Equiv (Fin N) (Fin N) := Equiv.addRight (1 : Fin N); have h_term3_eq_term2 : Finset.sum Finset.univ term3 = Finset.sum Finset.univ term2 := by { have : term3 = term2 âˆ˜ e := by { funext i; simp [term2, term3, Fin.cycle, e, Equiv.addRight, Fin.add_one_equiv_cycle hN, configToPath] }; rw [this]; exact Finset.sum_equiv e (by simp) (by simp) }; rw [h_term3_eq_term2, â† Finset.sum_add_distrib]; apply Finset.sum_congr rfl; intro i _; simp only [add_halves]; let n_i_r := fin2ToReal (if config i then 1 else 0); let n_ip1_r := fin2ToReal (if config (Fin.cycle hN i) then 1 else 0); have h_n_i_b : boolToReal (config i) = n_i_r := by simp [fin2ToReal, boolToReal]; have h_n_ip1_b : boolToReal (config (Fin.cycle hN i)) = n_ip1_r := by simp [fin2ToReal, boolToReal]; rw [h_n_i_b, h_n_ip1_b]; ring
def path_prod_full (p : FullParamSpace N) (s_path : Path N) : â„‚ := Finset.prod Finset.univ fun (i : Fin N) => (T_local_full N hN p i) (s_path i) (s_path (Fin.cycle hN i))
lemma trace_list_prod_rotate_induct (L : List (Matrix (Fin 2) (Fin 2) â„‚)) (n : â„•) : Matrix.trace (List.prod (L.rotate n)) = Matrix.trace (List.prod L) := by induction n with | zero => simp [List.rotate_zero] | succ k ih => cases L with | nil => simp | cons hd tl => simp only [List.rotate_cons_succ, List.prod_cons, Matrix.trace_mul_comm, â† List.prod_cons, â† List.rotate_cons]; exact ih
lemma trace_prod_reverse_eq_trace_prod (L : List (Matrix (Fin 2) (Fin 2) â„‚)) : Matrix.trace (List.prod L.reverse) = Matrix.trace (List.prod L) := by induction L using List.reverseRecOn with | H T M ih => rw [List.reverse_append, List.prod_append, List.prod_singleton, List.reverse_singleton, List.prod_cons, List.prod_append, List.prod_singleton, Matrix.trace_mul_comm (List.prod T) M]; exact ih | nil => simp
theorem trace_prod_reverse_eq_sum_path''' : Matrix.trace (T_prod_full N hN p) = Finset.sum Finset.univ (path_prod_full N hN p) := by let M := T_local_full N hN p; let L := List.ofFn M; rw [unfold T_prod_full, â† trace_prod_reverse_eq_trace_prod L, Matrix.trace_list_prod L]; apply Finset.sum_congr rfl; intro path _; unfold path_prod_full; apply Finset.prod_congr rfl; intro i _; simp only [List.get_ofFn]; congr 2; rw [Fin.cycle_eq_add_one hN i]; rfl
...
theorem Z_ED_K_eq_Z_TM_K_re : Z_ED_K N hN p.1 p.2.1 p.2.2 = (Z_TM_full N hN p).re := by
  ...
  rw [show Z_ED_K N hN p.1 p.2.1 p.2.2 = Finset.sum Finset.univ fun (c : Config N) => Complex.exp (â†‘(path_exponent_sum_full N hN p (configToPath N c)) : â„‚) by { rw [Complex.ofReal_sum]; apply Finset.sum_congr rfl; intro c _; rw [Complex.ofReal_exp, sum_exponent_eq_neg_H_full N hN p c]; rfl }]
  rw [trace_prod_reverse_eq_sum_path''' N hN p]; apply Finset.sum_congr rfl
  intro s_path _; unfold path_prod_full; simp_rw [T_local_full, Matrix.ofFn_apply]; rw [Complex.prod_exp]; Â· congr 1; exact path_exponent_sum_full N hN p s_path; Â· intros i _; apply Complex.exp_ne_zero
  rw [â† Finset.sum_equiv (configPathEquiv N).symm]; Â· intro path _; simp; Â· intros _ _; simp; Â· simp [configPathEquiv]; apply Finset.sum_congr rfl; intro config _; rfl
...
variable {N : â„•} (hN : 0 < N) (p : FullParamSpace N)

lemma Z_ED_pos (hp : p âˆˆ U_domain N) : 0 < Z_ED_K N hN p.1 p.2.1 p.2.2 := by unfold Z_ED_K; apply Finset.sum_pos; intro config _; apply Real.exp_pos; apply Finset.univ_nonempty_iff.mpr; exact Fintype.card_pos_iff.mp (by simp [Fintype.card_fun, Fin.card, hN, pow_pos])
lemma Z_TM_re_pos (hp : p âˆˆ U_domain N) : 0 < (Z_TM_full N hN p).re := by rw [â† Z_ED_K_eq_Z_TM_K_re N hN p]; exact Z_ED_pos N hN p hp
-- ContDiff Proof Chain (Proven)
lemma contDiff_T_local_exponent (i : Fin N) (idx1 idx2 : Fin 2) : ContDiff â„ âŠ¤ (T_local_exponent_full N hN i idx1 idx2) := by let proj_beta := ContinuousLinearMap.fst â„ â„ _; let proj_J_vec := (ContinuousLinearMap.fst â„ _ _).comp (ContinuousLinearMap.snd â„ â„ _); let proj_mu_vec := (ContinuousLinearMap.snd â„ _ _).comp (ContinuousLinearMap.snd â„ â„ _); let eval_J_i := EuclideanSpace.proj i; let eval_mu_i := EuclideanSpace.proj i; let eval_mu_ip1 := EuclideanSpace.proj (Fin.cycle hN i); have h_beta_cd := proj_beta.contDiff; have h_Ji_cd := eval_J_i.contDiff.comp proj_J_vec.contDiff; have h_mui_cd := eval_mu_i.contDiff.comp proj_mu_vec.contDiff; have h_muip1_cd := eval_mu_ip1.contDiff.comp proj_mu_vec.contDiff; have h_ni_cd : ContDiff â„ âŠ¤ (fun _ => fin2ToReal idx1) := contDiff_const; have h_nip1_cd : ContDiff â„ âŠ¤ (fun _ => fin2ToReal idx2) := contDiff_const; have h_two_cd : ContDiff â„ âŠ¤ (fun _ => (2:â„)) := contDiff_const; unfold T_local_exponent_full; dsimp; apply ContDiff.mul h_beta_cd; apply ContDiff.add; apply ContDiff.add; Â· apply ContDiff.mul h_Ji_cd (ContDiff.mul h_ni_cd h_nip1_cd); Â· apply ContDiff.mul; Â· apply ContDiff.div h_mui_cd h_two_cd (by norm_num); Â· exact h_ni_cd; Â· apply ContDiff.mul; Â· apply ContDiff.div h_muip1_cd h_two_cd (by norm_num); Â· exact h_nip1_cd
lemma contDiff_coe_real_complex : ContDiff â„ âŠ¤ (fun r : â„ => â†‘r : â„ â†’ â„‚) := Real.continuousLinearMap_ofReal.contDiff
lemma contDiff_cexp_real : ContDiff â„ âŠ¤ Complex.exp := Complex.contDiff_exp.restrictScalars â„
lemma contDiff_T_local_elem_real (i : Fin N) (idx1 idx2 : Fin 2) : ContDiff â„ âŠ¤ (fun p => T_local_full N hN p i idx1 idx2) := by apply ContDiff.comp contDiff_cexp_real; apply ContDiff.comp contDiff_coe_real_complex; exact contDiff_T_local_exponent N hN
