...
/-!
# Formal Verification of the 1D Inhomogeneous Lattice Gas Transfer Matrix

This file proves the exact equivalence between the partition function computed
via Exact Diagonalization (ED) and the Transfer Matrix (TM) method for a 1D
lattice gas with site-dependent couplings and potentials under Periodic
Boundary Conditions (PBC). It further proves the analyticity of the Free Energy
and the validity of thermodynamic derivative formulas for density and nearest-neighbor
correlations.
-/
...
abbrev KSpace (N : ℕ) := EuclideanSpace ℝ (Fin N) -- Space for variable couplings
abbrev MuSpace (N : ℕ) := EuclideanSpace ℝ (Fin N) -- Space for chemical potentials
...
variable {N : ℕ} (hN : 0 < N) (beta J : ℝ) (mu : Fin N → ℝ)
variable (K_vec : KSpace N) (mu_vec : MuSpace N) (p : FullParamSpace N)
variable (config : Config N) (path : Path N) (i j k : Fin N) (idx1 idx2 : Fin 2)
...
lemma Z_ED_pos : 0 < Z_ED_K N hN p.1 p.2.1 p.2.2 := by unfold Z_ED_K; apply Finset.sum_pos; intro config _; apply Real.exp_pos; apply Finset.univ_nonempty_iff.mpr; exact Fintype.card_pos_iff.mp (by simp [Fintype.card_fun, Fin.card, hN, pow_pos])
...
theorem F_TM_ContDiffOn : ContDiffOn ℝ ⊤ (F_TM N hN) (U_domain N) := by unfold F_TM; have h₁ : ContDiffOn ℝ ⊤ (fun p : FullParamSpace N => - (1 / p.1)) (U_domain N) := ContDiffOn.neg (ContDiffOn.inv (contDiff_fst.contDiffOn) (fun p hp => ne_of_gt hp)); have h₂ := contDiffOn_log_Z_tm N hN; exact ContDiffOn.mul h₁ h₂
...
theorem theorem1_density_verified (i : Fin N) (beta : ℝ)
    ...
    have h_log_deriv_filter : HasDerivAtFilter (log_Z_of_mu N hN beta J_vec)
           ...
           · exact hasDerivAtFilter_Z_ED_mu N hN i J_vec mu_vec beta h_beta_neq_zero h_Z_pos
    rw [HasDerivAtFilter.fderiv h_log_deriv_filter]
    simp only [ContinuousLinearMap.smul_apply, ContinuousLinearMap.comp_apply]
    rw [HasDerivAtFilter.fderiv (hasDerivAtFilter_Z_ED_mu N hN i J_vec mu_vec beta h_beta_neq_zero h_Z_pos)]
    field_simp [h_beta_neq_zero, ne_of_gt h_Z_pos]; ring

theorem theorem4_nn_correlation_verified (i : Fin N) (beta : ℝ)
    (h_beta_neq_zero : beta ≠ 0) (h_Z_pos : 0 < Z_ED_K N hN beta J_vec mu_vec)
    ...
    have h_log_deriv_filter : HasDerivAtFilter (log_Z_of_K N hN beta mu_vec)
           ...
           · exact Real.hasDerivAtFilter_log (ne_of_gt h_Z_pos)
           · exact hasDerivAtFilter_Z_ED_K N hN i J_vec mu_vec beta h_beta_neq_zero h_Z_pos
    rw [HasDerivAtFilter.fderiv h_log_deriv_filter]
    simp only [ContinuousLinearMap.smul_apply, ContinuousLinearMap.comp_apply]
    rw [HasDerivAtFilter.fderiv (hasDerivAtFilter_Z_ED_K N hN i J_vec mu_vec beta h_beta_neq_zero h_Z_pos)]
    field_simp [h_beta_neq_zero, ne_of_gt h_Z_pos]; ring
